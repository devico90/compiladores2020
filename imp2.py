...
from pi import run, EvaluationError
import sys
import traceback
...
def print_help():
    print(('Imπ2 to Π IR compiler and interpreter, August 2020', 'green'))
    print(('http://github.com/ChristianoBraga/PiFramework', 'grey'))
    print(('imp2.py -f <impfile>' +
           '[-s | -a | -d | -p | -t | --at | --pt | --stats | --state n | --last n | --rec | --no-color | --llvm | --llvm_jit]',
           'yellow'))
...
def main(argv):
    ...
    rec = False
    try:
        opts, args = getopt.getopt(argv, "f:saptb", [
                                   'at', 'pt', 'td', 'llvm', 'llvm_jit', 'stats', 'state=', 'last=', 'rec', 'no-color'])
    ...
    try:
        (tr, ns, dt) = run(pi_ast, rec=rec, color=color)
